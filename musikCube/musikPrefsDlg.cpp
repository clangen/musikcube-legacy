...
void CmusikPrefsInterfaceWorkflow::LoadPrefs()
{
	...
	ptrBtn->SetCheck( !musikCube::g_Prefs->AddEntireToNP() );
	ptrBtn = (CButton*)GetDlgItem( IDC_SHOWALLSONGS );
	ptrBtn->SetCheck( musikCube::g_Prefs->GetSelBoxShowAll() );
}
...
void CmusikPrefsInterfaceWorkflow::CommitChanges()
{
	...
	// selection box show all
	ptrBtn = (CButton*)GetDlgItem( IDC_SHOWALLSONGS );
	musikCube::g_Prefs->SetSelBoxShowAll( ptrBtn->GetCheck() );
...
BOOL CmusikPrefsAutoCapitalize::OnInitDialog()
{
	CmusikPropertyPage::OnInitDialog();
	LoadPrefs();
	return TRUE;
...
void CmusikPrefsInterfaceGeneral::CommitChanges()
{
    ...
    // global hot keys
    ptrBtn = (CButton*)GetDlgItem( IDC_CHECK_USEGLOBALKEYS );
    if ( ptrBtn->GetCheck() != musikCube::g_Prefs->GetUseGlobalHotkeys() )
    {
        musikCube::g_Prefs->SetUseGlobalHotkeys( ptrBtn->GetCheck() );
...
void CmusikPrefsInterfaceGeneral::OnBnClickedButton1()
{
    MessageBox(
        _T( "The following global hotkeys are used by musikCube:\n\n" )
		_T( "Ctrl + Alt + Numpad 5:  Play or Resume\n" )
		_T( "Ctrl + Alt + Numpad 0:  Stop\n\n" )
        _T( "Ctrl + Alt + Numpad 6:  Next Song\n" )
        _T( "Ctrl + Alt + Numpad 4:  Previous Song\n\n" )
        _T( "Ctrl + Alt + Numpad 8:  Volume Up\n" )
        _T( "Ctrl + Alt + Numpad 2:  Volume Down\n\n" )
        _T( "Ctrl + Alt + Numpad 7:  Previous Artist\n" )
        _T( "Ctrl + Alt + Numpad 9:  Next Artist\n\n" )
		_T( "Ctrl + Alt + Numpad 1:  Previous Album from Current Artist\n" )
        _T( "Ctrl + Alt + Numpad 3:  Next Album from Current Artist\n" ),
        _T( "musikCube" ),
        MB_ICONINFORMATION );
}
...
{
	CmusikSourcesCtrl::GetDevices( m_Drives );
...
BOOL CmusikPrefsInterfaceDevices::OnInitDialog()
{
	CmusikPropertyPage::OnInitDialog();
	LoadPrefs();
	return TRUE; 
...
BOOL CmusikPrefsInterfaceDevices::OnCommand(WPARAM wParam, LPARAM lParam)
{
	SetModified( TRUE );
	m_Modified = true;
...
BOOL CmusikPrefsInternet::OnInitDialog()
{
	CmusikPropertyPage::OnInitDialog();
	LoadPrefs();
	return TRUE; 
...
BOOL CmusikPrefsInternet::OnCommand(WPARAM wParam, LPARAM lParam)
{
	SetModified( TRUE );
	m_Modified = true;
	return CmusikPropertyPage::OnCommand(wParam, lParam);
