...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_ARTIST );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_ALBUM );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_YEAR );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_GENRE );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_TRACKNUM );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_TIMEADDED );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_LASTPLAYED );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_FORMAT );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_RATING );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_TIMESPLAYED );
	ReqSelBoxUpdate();
}
...
{
	GetCtrl()->SetType( musikCore::MUSIK_LIBRARY_TYPE_BITRATE );
	ReqSelBoxUpdate();
}
...
void CmusikSelectionBar::ReqSelBoxUpdate()
{
	...
	GetCtrl()->m_Parent->PostMessage( WM_SELBOXREQUESTUPDATE );
}
...
void CmusikSelectionCtrl::UpdateV( bool requery )
{
	if ( requery )
	{
		...
		if ( m_Type == musikCore::MUSIK_LIBRARY_TYPE_RATING )
		{
			...
			musikCube::g_Library->GetAllDistinct( m_Type, m_Items, true );
...
void CmusikSelectionCtrl::UpdateV( const musikCore::Playlist& source )
{
	int nPos = GetScrollPos( SB_VERT );
	...
	else
	{
		...
		musikCube::g_Library->GetRelatedItems( 
			m_Type, 
			m_Items, 
			info
		);
...
void CmusikSelectionCtrl::UpdateV( musikCore::String& query, const musikCore::Playlist& source )
{
	int nPos = GetScrollPos( SB_VERT );
...
void CmusikSelectionCtrl::UpdateV( musikCore::String query )
{
	m_ProtectingUpdate.lock();
	...
	bool sub_query = true;
	if ( query.Left( 1 ) == _T( "W" ) )
		sub_query = false;

	m_Items.clear();
	musikCube::g_Library->GetRelatedItems( query, m_Type, m_Items, sub_query );
...
musikCore::String CmusikSelectionCtrl::GetSelQuery()
{
...
void CmusikSelectionCtrl::OnNMCustomdraw(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	if ( pLVCD->nmcd.dwDrawStage == CDDS_ITEMPREPAINT )
	{
		...
		if ( m_Type == musikCore::MUSIK_LIBRARY_TYPE_RATING )
		{
			if ( item_str != _T( "0" ) && item_str != _T( "Unrated" ) )
			{
...
void CmusikSelectionCtrl::BeginDrag( bool right_button )
{
	...
	{
	// if the visible playlist is standard or dynamic,
	// we need to query it by building a statement like 
	// this ... AND field IN (...)
	case musikCore::MUSIK_PLAYLIST_TYPE_STANDARD:
	case musikCore::MUSIK_PLAYLIST_TYPE_DYNAMIC:
		{
			musikCore::String and_stmt = _T( " AND " );
			and_stmt += GetTypeDB();
			and_stmt += _T( " IN ( " );
			...
			musikCube::g_Library->GetRelatedItems( 
				...
				files, 
				musikCube::g_PlaylistInfo, 
				and_stmt 
			);
		}
		break;
	default:
		musikCube::g_Library->GetRelatedItems( GetType(), sel, musikCore::MUSIK_LIBRARY_TYPE_FILENAME, files );
		break;
	}
...
void CmusikSelectionCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    {
        if ( m_SearchBuf != _T( "" ) )
            m_SearchBuf.resize( m_SearchBuf.size() - 1 );
        else
        {
            UpdateV();
            return;
    ...
    query.Format(
        ...
        _T( " SELECT DISTINCT %s " ) 
        _T( " FROM %s " )
        _T( " WHERE LOWER(%s) LIKE LOWER('%%%s%%') " )
        _T( " ORDER BY LOWER(%s); " ),

        musikCube::g_Library->GetSongFieldStr( m_Type ).c_str(),
        SONG_TABLE_NAME,
        musikCube::g_Library->GetSongFieldStr( m_Type ).c_str(),
        m_SearchBuf.c_str(),
        musikCube::g_Library->GetSongFieldStr( m_Type ).c_str() );

    musikCube::g_Library->RawQueryStringArray(
        query,
        m_Items );
	...
	m_Parent->SendMessage( WM_SELBOXFILTER, (WPARAM)this );
}
...
void CmusikSelectionCtrl::DelSel( bool from_file )
{
	...
	m_Parent->PostMessage( WM_SELBOXDELSEL, (WPARAM)this, (LPARAM)from_file );
}
...
void CmusikSelectionCtrl::StoreCache()
{
	m_Cache.m_SelItems.clear();
	m_Cache.m_Items.clear();

	GetSelItems( m_Cache.m_SelItems );

	m_Cache.m_Items = m_Items;
	m_Cache.m_Parent = IsParent();
	m_Cache.m_ChildOrder = GetChildOrder();	
	m_Cache.m_ScrollPos = GetScrollPos( SB_VERT );
}

///////////////////////////////////////////////////

void CmusikSelectionCtrl::RestoreCache()
{
	// have initialized items
	if ( m_Cache.m_Items.size() )
	{
		m_Items = m_Cache.m_Items;
		UpdateV( false );
	}
	...
	for ( size_t i = 0; i < m_Cache.m_SelItems.size(); i++ )
	{
		for ( size_t j = 0; j < m_Items.size(); j++ )
		{
			if ( m_Cache.m_SelItems.at( i ) == m_Items.at( j ) )
			{
	...
	m_ParentBox = m_Cache.m_Parent;
	m_ChildOrder = m_Cache.m_ChildOrder;
	SetScrollPos( SB_VERT, m_Cache.m_ScrollPos, 0 );
...
void CmusikSelectionCtrl::OnNMKillfocus(NMHDR *pNMHDR, LRESULT *pResult)
{
    m_SearchBuf.clear();
    UpdateWindowCap( false );
    *pResult = 0;
...
{
	musikCore::String query = GetCtrl()->GetSelQuery();
	musikCore::Playlist* playlist = new musikCore::Playlist();
	musikCube::g_Library->QuerySongs( query, *playlist );
