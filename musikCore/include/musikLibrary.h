...
public:
	 ...
	 */
	Library( const String& filename, bool use_utc = true );
	~Library();
	 ...
	 */
	static void DeleteSongs( Playlist& songs, bool delete_from_disk = false );
	void DeleteSong( String& fn, bool delete_from_disk = false );
	...
	*/
	...
	/**
	 * A general purpose query tool that can be used to find related field values.
	 *
	 * Here is a practical example: 
	 *
	 * You have a list of artists, lets say "Jethro Tull" and "The Beatles", and you would
	 * like to find all the albums in the library by these two artists.
	 * 
	 *     1. source_type would be MUSIK_LIBRARY_TYPE_ARTIST\n
	 *     2. source_items would be a StringArray containing two items, "Jethro Tull" and
	 *       "The Beatles"\n
	 *     3. target_type would be MUSIK_LIBRARY_TYPE_ALBUM, because we want to find all albums
	 *       related to the specified artists.\n
	 *	   4. target would be the address of a string array to receive the related information.\n\n
	 *
	 * \param source_type type of items that are contained within source_items. MUSIK_LIBRARY_TYPE_xxx
	 * \param source_items an array of items containing fields we wish to find relational material for
	 * \param target_type library type we are querying for. MUSIK_LIBRARY_TYPE_xxx
	 * \param target array to receive related material
	 * \return MUSIK_LIBRARY_OK if successful, error code otherwise
	 */
	int GetRelatedItems( int source_type, const StringArray& sources_items, int target_type, StringArray& target );
	
	int GetRelatedSongs( String partial_query, int order_by, PlaylistInfo& info, Playlist& target, bool clear_target = true );
	int GetRelatedItems( int target_type, StringArray& target, PlaylistInfo& source, const musikCore::String& and_stmt = _T( "" ) );
	...
	/**
	 * A general purpose query tool that can be used to find related field values.
	 * This function differs with its overloaded partner with the ability to perform subquerying.
	 * 
	 ...
	 * \return MUSIK_LIBRARY_OK if successful, error code otherwise
	 */
	int GetRelatedItems( String partial_query, int order_by, StringArray& target, bool sub_query = false );
	/**
	 ...
	 * \return MUSIK_LIBRARY_OK if successful, error code otherwise
	 */
	int GetDirtySongs( Playlist& target, bool clear = true );
	/**
	 ...
	 */
	void SetUseUTC( bool use_utc ){ m_UTC = use_utc; }
	...
	int	RawQuerySongs( const String & query, Playlist & target );
    int RawQueryStringArray( const String& query, StringArray& target, bool clear = true );
...
protected:
	...
	*/
	...
	String GetOrder( int type, bool terminate = true, bool by_upper = false, bool descend = false );
...
public:
	...
	RemLibrary( const String& filename, bool use_utc = true );
