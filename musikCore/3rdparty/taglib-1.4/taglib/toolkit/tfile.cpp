...
{
public:
  FilePrivate(const wchar_t *fileName) :
    file(0),
    name(fileName),
    readOnly(true),
    valid(true),
...
{
  d = new FilePrivate(::_wcsdup(file));

  d->readOnly = !isWritable(file);
  d->file = ::_wfopen(file, d->readOnly ? L"rb" : L"r+b" );
...
bool File::isReadable(const wchar_t *file)
{
...
bool File::isWritable(const wchar_t *file)
{
	FILE* pFile = _wfopen( file, L"r+b" );
	if ( pFile )
	{
		fclose( pFile );
		return true;
