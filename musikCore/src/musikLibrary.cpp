...
Library::Library( const String& filename, bool use_utc )
{
	srand( time( 0 ) );
...
int Library::GetStdPlaylist( int id, Playlist& target, bool clear_target )
{
	...
	query.Format(
		...
		_T( "   SELECT songid " )
		_T( "   FROM ( " )
		_T( "     SELECT std_playlist_songid,songid " )
		_T( "     FROM std_playlist_song " )
		_T( "     WHERE std_playlist_id = %d " )
		_T( "     ORDER BY std_playlist_songid " )
		_T( "   ) " )
		_T( " ); " ),
		//_T( " ORDER BY artist, album, tracknum, filename; " ),
		id );
	*/
	...
	//Claim( target );  <-- done by RawQuerySongs()
...
int Library::GetAllSongs( Playlist& target )
{
	...
	if ( !nRet )
	{
		target.SetType( MUSIK_PLAYLIST_TYPE_LIBRARY );
	}
	*/

	//Claim( target );   <-- done by QuerySongs()
...
int Library::RawQueryStringArray( const String& query, StringArray& target, bool clear )
{
	if ( !m_DatabaseOpen )
		return MUSIK_LIBRARY_NOT_OPEN;
...
int Library::RawQuerySongs( const String& query, Playlist& target )
{
	if ( !m_DatabaseOpen )
		return MUSIK_LIBRARY_NOT_OPEN;
...
int Library::GetRelatedItems( int source_type, const StringArray& source_items, int target_type, StringArray& target )
{
	if ( !m_DatabaseOpen )
		return MUSIK_LIBRARY_NOT_OPEN;
	...
	const String sInType = GetSongFieldDBStr( source_type );
	const String sOutType = GetSongFieldDBStr( target_type );

	// construct the query
	String query;
	query.Format( _T( "SELECT DISTINCT %ls, UPPER( %ls ) AS UP FROM %ls where " ), 
		sOutType.c_str(), 
		sOutType.c_str(), 
		SONG_TABLE_NAME );
	...
	int nRet;
	m_ProtectingLibrary.lock();
		nRet = sqlite3_exec( m_pDB, utf16to8( query ).c_str(), &sqlite_AddSongToStringArray, &target, NULL );
	m_ProtectingLibrary.unlock();
...
int Library::GetRelatedItems( int target_type, StringArray& target, PlaylistInfo& info, const String& and_stmt )
{
	if ( !m_DatabaseOpen )
		return MUSIK_LIBRARY_NOT_OPEN;

	String intype = GetSongFieldDBStr( target_type );

	String pre_stmt;
	if ( info.GetType() == musikCore::MUSIK_PLAYLIST_TYPE_STANDARD )
	{
		pre_stmt.Format(
			_T( " WHERE songid IN ( SELECT songid FROM std_playlist_song WHERE std_playlist_id = %d ) " ),
			info.GetID() );
	}
...
int Library::GetRelatedItems( String partial_query, int dst_type, StringArray& target, bool sub_query )
{
	target.clear();

	String sOutType = GetSongFieldDBStr( dst_type );
    String query;
...
int Library::GetAllStdPlaylists( PlaylistInfoArray& target, bool clear_target )
{
	...
	m_ProtectingLibrary.lock();
		...
		nRet = sqlite3_exec( 
            m_pDB, 
            utf16to8( query ).c_str(), 
			&sqlite_AddStdPlaylistInfoArray, 
            &target, 
            NULL );
...
int Library::GetAllDynPlaylists( PlaylistInfoArray& target, bool clear_target )
{
	...
	m_ProtectingLibrary.lock();
		...
		nRet = sqlite3_exec( 
                m_pDB, 
                utf16to8( query ).c_str(),
			    &sqlite_AddDynPlaylistInfoArray, 
                &target, 
                NULL );
...
bool Library::AddSong( const String& fn )
{
	...
	if ( !IsSongInLibrary( fn ) )
	{
		...
		else
		{
			...
			{
				if ( PlayerBASS::s_Plugins.at( i ).CanPlay( fn ) )
				{
					SongInfo info;
					info.SetLibrary( this );
					info.SetFilename( fn );
					PlayerBASS::s_Plugins.at( i ).ReadTag( fn, info );
					result = AddFile( info );
					break;
				}
			}
...
int Library::GetSongFormatFromID( int id, String& target )
{
	...
	nRet = sqlite3_exec( 
        m_pDB, 
        utf16to8( query ).c_str(), 
		&sqlite_GetStringFromRow, &target, NULL );
...
int Library::GetDirtySongs( Playlist& target, bool clear )
{
	if ( clear )
		target.Clear();

	int nRet = QuerySongs( "dirty = 1", target );
	if ( !nRet )
...
void Library::DeleteSong( String& fn, bool delete_from_disk )
{
	m_ProtectingLibrary.lock();
	...
	if ( delete_from_disk && Filename::FileExists( fn ) )
	{
#if defined (WIN32) || defined (__MINGW32__)
		_wremove( fn.c_str() );
#else
		remove( utf16to8( fn ).c_str() );
#endif
	}
}
...
void Library::DeleteSongs( Playlist& songs, bool delete_from_disk )
{
	String filename;
	...
	for ( size_t i = 0; i < songs.GetCount(); i++ )
	{
		...
		curr->DeleteSong( filename, delete_from_disk );
	}
...
int Library::GetAllFns( String& items )
{
	...
	m_ProtectingLibrary.lock();
		int ret = sqlite3_exec(
			...
			sqlite_AppendFileStringFromRow, &items, NULL );
	m_ProtectingLibrary.unlock();
	items += _T( "\0" );
...
RemLibrary::RemLibrary( const String& filename, bool use_utc )
: Library( filename, use_utc )
{
	InitRemPathTable();
}
