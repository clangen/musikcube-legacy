...
void ShibatchEQ::equ_init(int wb)
{
  int i,j;
...
// -(N-1)/2 <= n <= (N-1)/2
REAL ShibatchEQ::win(REAL n,int N)
{
...
void ShibatchEQ::equ_makeTable(REAL *lbc,REAL *rbc,paramlist *param,REAL fs)
{
...
void ShibatchEQ::equ_quit(void)
{
  if(lires1)free(lires1);
  if(lires2)free(lires2);
  if(rires1)free(rires1);
  if(rires2)free(rires2);
  if(irest)free(irest);
  if(fsamples)free(fsamples);
  if(inbuf)free(inbuf);
  if(outbuf)free(outbuf);
  if(ditherbuf)free(ditherbuf);
...
void ShibatchEQ::equ_clearbuf(int bps,int srate)
{
	int i;
...
int ShibatchEQ::equ_modifySamples(char *buf,int nsamples,int nch,int bps)
{
  int i,p,ch;
  REAL *ires;
  int amax =  (1 << (bps-1))-1;
